...
struct KudoSenderReceiverHighlight: View {
    ...
    /// When non-nil the sender slot renders an anonymous mask + nickname
    /// instead of the real avatar/employee-code/badge. Mirrors the
    /// "View kudo ẩn danh" Figma frame (mm:6885:10194).
    let anonymousNickname: String?

    @EnvironmentObject private var localizer: Localizer

    init(sender: KudosUser, receiver: KudosUser, anonymousNickname: String? = nil) {
        self.sender = sender
    ...
    private static let anonymousMaskBg = Color(red: 0xD0 / 255.0, green: 0xD0 / 255.0, blue: 0xD0 / 255.0)

    var body: some View {
        // mm:6885:10149 — trao nhận row: sender | arrow | receiver
        HStack(alignment: .top, spacing: 8) {
            // mm:I6885:10150 — Infor (sender). Anonymous swaps to the mask block.
            if let nickname = anonymousNickname {
                anonymousSenderBlock(nickname: nickname)
            } else {
    ...
    // MARK: - Anonymous sender block (mm:6885:10194)

    private func anonymousSenderBlock(nickname: String) -> some View {
        VStack(alignment: .leading, spacing: 4) {
            ZStack {
                Circle()
                    .fill(Self.anonymousMaskBg)
                    .frame(width: 40, height: 40)
                Image(systemName: "person.fill.questionmark")
                    .font(.system(size: 18))
                    .foregroundColor(Self.textDark.opacity(0.7))
            }
            ...
            Text(localizer.t("kudos.detail.sender.anonymous"))
                .font(.custom("Montserrat", size: 10))
                .fontWeight(.medium)
                .foregroundColor(Self.textMid)
                .lineLimit(1)
