...
| 2 | Kudos | `KudosTabViewContainer → KudosTabView` | `heart.fill` |
...
| `KudosService` | `FakeKudosService` | 9 methods: `loadKudos`, `loadHighlight`, `loadKudosFeed`, `loadHashtags`, `loadDepartments`, `loadKudosStats`, `loadTopRecipients`, `loadSpotlightCount`, `openNextSecretBox` | REST kudos endpoints |
...
│   ├── Kudos/
│   │   ├── KudosTabView.swift        # KudosTabViewContainer + full KudosTabView (scrollable layout)
│   │   ├── KudosOverviewView.swift   # stub (view-all destination)
│   │   ├── KudosFeedView.swift       # stub (detail destination)
│   │   ├── KudosViewModel.swift      # @MainActor ObservableObject; LoadState<Void>; 9 service calls
│   │   ├── Models/
...
│   │       ├── KudosFilterRow.swift  # Menu pickers for hashtag + department
│   │       ├── SectionHeader.swift
│   │       ├── HashtagChip.swift
│   │       ├── KudosCard.swift       # heart toggle; carousel + feed variants
│   │       ├── HighlightCarousel.swift
│   │       ├── SpotlightBoard.swift  # static placeholder + search field
...
│   ├── KudosService.swift            # protocol (9 methods) + FakeKudosService
│   ├── KudosFixtures.swift           # single source of mock truth for Kudos screen
